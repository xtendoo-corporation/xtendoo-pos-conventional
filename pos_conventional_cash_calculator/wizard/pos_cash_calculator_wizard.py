...
class PosCashCalculatorWizard(models.TransientModel):
    ...
    # ── Referencia al wizard padre ────────────────────────────────────────────
    parent_model = fields.Selection(
        selection=[
            ("pos.session.closing.wizard", "Wizard de cierre"),
            ("pos.session.cash_move.wizard", "Wizard de entrada/salida de efectivo"),
    ...
    def action_confirm(self):
        ...
        if self.parent_model == "pos.session.cash_move.wizard" and parent_wizard:
            ...
            return self._get_parent_action(parent_wizard)
        if self.parent_model == "pos.session" and parent_wizard:
            parent_wizard.write({
    ...
    def action_clear(self):
        self.ensure_one()
        self.write({
            "qty_200": 0, "qty_100": 0, "qty_50": 0, "qty_20": 0,
            "qty_10": 0, "qty_5": 0, "qty_2": 0, "qty_1": 0,
            "qty_050": 0, "qty_020": 0, "qty_010": 0, "qty_005": 0,
            "qty_002": 0, "qty_001": 0,
        })
    ...
    def increment_001(self): self.qty_001 += 1; return self._reload_view()
    ...
    def decrement_001(self):
        if self.qty_001 > 0: self.qty_001 -= 1
        return self._reload_view()
