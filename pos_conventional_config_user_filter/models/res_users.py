...
from ..hooks import POS_USER_CORE_RULES
...
class ResUsers(models.Model):
    ...
    def _register_hook(self):
        ...
        # Varias reglas del core (grupo POS User) permiten ver cualquier
        # factura/línea/apunte bancario ligado a CUALQUIER caja. Al
        # compartir grupo con nuestras propias reglas, ir.rule las combina
        # con OR, así que no pueden acotarse añadiendo una regla nueva: hay
        # que sobrescribir su domain_force directamente. Son noupdate=True
        # (no se puede vía XML), y por eso se aplica aquí, en cada carga del
        # registro (arranque o -u), en vez de solo en la instalación inicial
        # (ver también uninstall_hook, y el detalle en hooks.py).
        for xmlid, domains in POS_USER_CORE_RULES.items():
            rule = self.env.ref(xmlid, raise_if_not_found=False)
            if rule and rule.domain_force != domains["restricted"]:
                rule.sudo().write({"domain_force": domains["restricted"]})
